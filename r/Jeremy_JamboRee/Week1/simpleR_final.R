...
p = df %>% #run this. Nothing seems to happen because the plot is placed into the variable p rather than being spit out on our screens
  ggplot(aes(x=1:length(df[,1]), y=numbers)) +
  geom_point()
...
p3 = df %>% #run this. Nothing seems to happen because the plot is placed into the variable p rather than being spit out on our screens
